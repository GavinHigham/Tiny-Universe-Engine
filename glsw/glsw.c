...
int glswInit()
{
    ...
    __glsw__Context = (glswContext*) calloc(1, sizeof(glswContext));
    ...
    if (!pLoadedEffect)
    {
...

        {
            ...
            // Add a new entry to the front of gc->LoadedEffects
            {
                ...
                gc->LoadedEffects = (glswList*) calloc(1, sizeof(glswList));
            ...
            if (blength(line) >= 2 && line->data[0] == '-' && line->data[1] == '-')
            {
                ...
                else
                {
                    ...
                    // Add a new entry to the shader map.
                    {
                        ...
                        gc->ShaderMap = (glswList*) calloc(1, sizeof(glswList));
...
int glswAddDirectiveToken(const char* token, const char* directive)
{
    ...
    temp = gc->TokenMap;
    gc->TokenMap = (glswList*) calloc(1, sizeof(glswContext));
